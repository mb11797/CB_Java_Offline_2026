...
public class SingleSourceShortestPathAlgorithm{
	...
	public static int[] singleSourceShortestPathUsingBFS(List<Integer>[] adjListGraph, int src){
		...
		while(!q.isEmpty()){
			...
			for(int nbr: adjListGraph[front]){
	...
	public static void main(String[] args) throws IOException{	
		...
		int[] dist = singleSourceShortestPathUsingBFS(adjListGraph, src);

		for(int node=0; node<dist.length; node++){
			System.out.println("Distance from " + src + " to " + node + " = " + dist[node]);
		}
		System.out.println();
